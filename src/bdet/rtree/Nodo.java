...
public class Nodo extends Entrada {
    ...
    private Estadistica estadistica; // Estadísticas de lectura
    ...
    public Nodo insertar(Entrada e) {
        ...
        if (esHoja) {
            ...
            if (e.hilbert > hilbert) {
                ...
                for (n = 0; n < numeroHijos; n++) // Selecciona el hijo en que mejor se acomoda la entrada
                {
    ...
    private void adoptar(Nodo vecino, int num) {
        ...
        if (hilbert <= vecino.hilbert) {
            ...
            for (i = numeroHijos - 1; i >= 0; i--) // Rota los hijos
            {
                hijos[i + num] = hijos[i];
            }
